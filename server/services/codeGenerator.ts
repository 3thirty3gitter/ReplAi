...
export async function generateProjectFiles(request: CodeGenerationRequest): Promise<CodeGenerationResponse> {
  ...
  // Use Perplexity to generate complete application architecture
  const architectureResponse = await getCodeAssistance({
    code: '',
    language: 'typescript',
    prompt: `As a senior full-stack developer, create a complete production-ready application for: "${request.prompt}". 

    Generate:
    1. A React TypeScript application with proper components, hooks, and state management
    2. Express.js backend with RESTful API endpoints
    3. PostgreSQL database schema with Drizzle ORM
    4. Full authentication and user management
    5. Modern UI with Tailwind CSS and responsive design

    Provide actual working code, not templates or placeholders. The application should be fully functional with real business logic, proper error handling, and production-ready architecture.`,
    context: 'Full-stack application generation'
  });

  // Generate React frontend using Perplexity
  const frontendResponse = await getCodeAssistance({
    code: '',
    language: 'typescript',
    prompt: `Create a complete React TypeScript frontend application for: "${request.prompt}". 
    ...
    - Real business logic and user interactions
    - Error handling and loading states

    Generate the main App.tsx file with complete functionality.`,
    context: 'React frontend generation'
  });

  // Generate backend API using Perplexity
  const backendResponse = await getCodeAssistance({
    code: '',
    language: 'typescript',
    prompt: `Create a complete Express.js backend for: "${request.prompt}".
    ...
    Generate the routes.ts file with complete API functionality.`,
    context: 'Express backend generation'
  });

  // Generate database schema using Perplexity
  const schemaResponse = await getCodeAssistance({
    code: '',
    language: 'typescript',
    prompt: `Create a complete PostgreSQL database schema for: "${request.prompt}".
    ...
    - Business-specific tables and relationships

    Generate the schema.ts file using Drizzle ORM with complete database design.`,
    context: 'Database schema generation'
  });
  ...
  const files: GeneratedFile[] = [
    {
      ...
      content: frontendResponse.suggestion || 'Error generating frontend code',
      language: 'typescript',
    ...
    },
    {
      ...
      content: backendResponse.suggestion || 'Error generating backend code',
      language: 'typescript',
    ...
    },
    {
      ...
      content: schemaResponse.suggestion || 'Error generating schema code',
      language: 'typescript',
      description: 'Database schema generated by Perplexity AI'
    }
  ];

  await createGeneratedFiles(request.projectId, files);

  return {
    files,
    instructions: architectureResponse.explanation || 'Application generated using Perplexity AI',
    nextSteps: [
      'Review Perplexity-generated code',
      'Test application functionality',
      'Deploy when ready'
    ],
    dependencies: [
      'react', 'typescript', 'tailwindcss', 'wouter', '@tanstack/react-query',
      'express', 'drizzle-orm', 'zod', 'react-hook-form'
    ]
  };
}
...
async function generateFullStackApplication(prompt: string, appType: ApplicationType, projectId: number): Promise<GeneratedFile[]> {
  ...
  switch (appType) {
    ...
    case 'saas-platform':
      return generateDashboardApp(prompt, projectId);
    case 'health-fitness':
      throw new Error('Template-based generation removed. Use Perplexity-only generation via generateProjectFiles.');
    default:
      throw new Error('Template-based generation removed. Use Perplexity-only generation via generateProjectFiles.');
  }
}
...
// This function is no longer needed - all generation is now handled directly by Perplexity in generateProjectFiles
